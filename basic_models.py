...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1,
                 classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, full_gru_ode=False,
                 solver="euler", impute=True, **options):
        """
    ...
    def ode_step(self, h, p, delta_t, current_time):
        ...
        if self.solver == "euler":
            ...
            solution, eval_times, eval_vals = odeint(self.gru_c, h, torch.tensor([0, delta_t]), method=self.solver,
    ...
    def forward(self, times, time_ptr, X, M, obs_idx, delta_t, T, cov,
        ...
        for i, obs_time in enumerate(times):
            ...
            loss_2 = loss_1 + compute_KL_loss(p_obs=p[i_obs], X_obs=X_obs, M_obs=M_obs, logvar=self.logvar)
        ...
        if return_path:
            if smoother:
                ...
                print('####: ', eval_times_total, eval_vals_total)
                return h, loss, class_pred, np.array(path_t), torch.stack(path_p), torch.stack(
                    path_h), eval_times_total, eval_vals_total
        else:
            if smoother:
...
class GRUODEBayesSeq(torch.nn.Module):
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1,
                 classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, obs_noise_std=1e-2,
                 full_gru_ode=False):
        super().__init__()
        ...
        self.gru_bayes = SeqGRUBayes(input_size=input_size, hidden_size=hidden_size, prep_hidden=prep_hidden,
                                     p_hidden=p_hidden, bias=bias)
    ...
    def forward(self, times, time_ptr, Xpadded, Fpadded, X, M, lengths,
        ...
        for i, obs_time in enumerate(times):
            ...
            loss_2 = loss_2 + compute_KL_loss(p_obs=p[i_obs], X_obs=Xf_batch, M_obs=Mf_batch,
...
class Discretized_GRU(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, p_hidden, prep_hidden, bias=True, cov_size=1, cov_hidden=1,
                 classification_hidden=1, logvar=True, mixing=1, dropout_rate=0, impute=True):
        """
        The smoother variable computes the classification loss as a weighted average of the projection of the latents at each observation.
        """
